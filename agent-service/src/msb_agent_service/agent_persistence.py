...
class AgentPersistenceRepository:
    ...
    async def apply_retention(
        ...
        *,
        ...
        deleted_discovery_exclusions = 0
        redacted_discovery_exclusion_commands = 0
        redacted_keys = 0
        async with self._pool.acquire() as connection:
            try:
                await connection.begin()
                async with connection.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    for session_id in session_ids:
                        ...
                        await cursor.execute(
                            """
                            DELETE FROM agent_discovery_exclusions
                            WHERE session_id = %s
                            """,
                            (session_id,),
                        )
                        deleted_discovery_exclusions += cursor.rowcount
                        await cursor.execute(
                            """
                            UPDATE agent_discovery_exclusion_commands
                            SET response_listing_id = NULL,
                                response_reason_code = NULL,
                                content_redacted_at = %s
                            WHERE session_id = %s
                              AND content_redacted_at IS NULL
                            """,
                            (_mysql_datetime(current), session_id),
                        )
                        redacted_discovery_exclusion_commands += cursor.rowcount
                        await cursor.execute(
                            """
                    ...
                    await cursor.execute(
                        ...
                        LIMIT %s
                        """,
                        (_mysql_datetime(current), audit_batch_size),
                    )
                    deleted_discovery_exclusion_commands = cursor.rowcount
                await connection.commit()
            except Exception:
                await connection.rollback()
                raise
        ...
        self._metrics.record_retention(
            "discovery_exclusions_deleted",
            deleted_discovery_exclusions,
        )
        self._metrics.record_retention(
            "discovery_exclusion_commands_redacted",
            redacted_discovery_exclusion_commands,
        )
        self._metrics.record_retention(
            "discovery_exclusion_commands_deleted",
            deleted_discovery_exclusion_commands,
        )
        ...
        if purged_sessions or deleted_audit:
            LOGGER.info(
                ...
                "discoveryExclusionsDeleted=%s "
                "discoveryExclusionCommandsRedacted=%s "
                "discoveryExclusionCommandsDeleted=%s "
