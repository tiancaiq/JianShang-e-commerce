...
class DiscoveryPersistenceIntegrationTest(unittest.IsolatedAsyncioTestCase):
    """Proves the V7/V8 discovery contracts against a real MySQL engine."""
    ...
    async def asyncSetUp(self) -> None:
        ...
        try:
            ...
            await self._apply_migrations(from_version=7, through_version=8)
    ...
    async def test_v7_v8_apply_after_v1_through_v6_and_validate_schema(self) -> None:
        ...
        async with self.repository.pool.acquire() as connection:
            try:
                async with connection.cursor() as cursor:
                    ...
                    exclusion_tables = int((await cursor.fetchone())[0])
            finally:
        ...
        self.assertEqual(2, exclusion_tables)

    async def test_exclusion_replay_conflict_isolation_and_zero_history(self) -> None:
        ...
        recommendation = _recommendation_response("X")
        ...
        listing_id = recommendation.recommendations[0].listing_id
        key = "exclude-integration-0001"
        first = await self.discovery.apply_exclusion(
            session_id=session.session_id,
            actor_user_id=ACTOR_A,
            idempotency_key=key,
        ...
        replay = await self.discovery.apply_exclusion(
            session_id=session.session_id,
            actor_user_id=ACTOR_A,
            idempotency_key=key,
            expected_preference_version=1,
        ...
        self.assertEqual(2, first.preference_version)
        self.assertEqual(1, first.excluded_count)
        self.assertEqual(2, await self._scalar(
            "SELECT COUNT(*) FROM agent_messages WHERE session_id = %s",
            (session.session_id,),
        ))
        with self.assertRaises(DiscoveryExclusionPersistenceError) as changed:
            await self.discovery.apply_exclusion(
                session_id=session.session_id,
                actor_user_id=ACTOR_A,
                idempotency_key=key,
        ...
        with self.assertRaises(DiscoveryExclusionPersistenceError) as hidden:
            await self.discovery.apply_exclusion(
                session_id=session.session_id,
                actor_user_id=ACTOR_B,
                idempotency_key="exclude-integration-0002",
                expected_preference_version=2,
                listing_id=listing_id,
                reason_code=None,
    ...
    async def test_exclusion_same_key_concurrency_and_new_search_isolation(self) -> None:
        session = await self._session()
        begun = await self._begin(
            session.session_id,
        ...
        recommendation = _recommendation_response("Y")
        await self.discovery.complete_turn(
            invocation_id=begun.invocation.invocation_id,
            session_id=session.session_id,
            actor_user_id=ACTOR_A,
        ...
        kwargs = {
            "session_id": session.session_id,
            "actor_user_id": ACTOR_A,
            "idempotency_key": "exclude-concurrent-0001",
            "expected_preference_version": 1,
            "listing_id": recommendation.recommendations[0].listing_id,
            "reason_code": None,
            "now": NOW + timedelta(seconds=1),
        ...
        first, second = await asyncio.gather(
            self.discovery.apply_exclusion(**kwargs),
            self.discovery.apply_exclusion(**kwargs),
        )
        self.assertEqual(first, second)
        self.assertEqual(1, await self._scalar(
            "SELECT COUNT(*) FROM agent_discovery_exclusions",
        ))
        replacement, _ = await self.discovery.create_or_resume(
            actor_user_id=ACTOR_A,
            new_search=True,
            now=NOW + timedelta(seconds=2),
        )
        self.assertEqual(
            (),
            await self.discovery.list_exclusions(
    ...
    async def test_complete_turn_is_atomic_and_recommendations_survive_restart(
        ...
        latest = await self.discovery.latest_recommendations(
            session_id=session.session_id,
            actor_user_id=ACTOR_A,
        )
        self.assertEqual(
            tuple(item.listing_id for item in response.recommendations),
            tuple(item.listing_id for item in latest),
        )
        self.assertEqual(
            (),
            await self.discovery.latest_recommendations(
                session_id=session.session_id,
                actor_user_id=ACTOR_B,
            ),
        )

        compared = DiscoveryTurnResponse(
            outcome="COMPARE",
            message="Compared two current listings.",
            recommendations=response.recommendations[:2],
            preferenceState=response.preference_state,
        )
        comparison = await self._begin(
            session.session_id,
            "01ARZ3NDEKTSV4RRFFQ69G5FC1",
            "Compare the first two",
            now=NOW + timedelta(seconds=2),
        )
        compared_session, _ = await self.discovery.complete_turn(
            invocation_id=comparison.invocation.invocation_id,
            session_id=session.session_id,
            actor_user_id=ACTOR_A,
            expected_preference_version=1,
            response=compared,
        ...
        self.assertEqual(2, compared_session.preference_version)
        after_comparison = await self.discovery.latest_recommendations(
            session_id=session.session_id,
            actor_user_id=ACTOR_A,
        )
        self.assertEqual(
            tuple(item.listing_id for item in compared.recommendations),
            tuple(item.listing_id for item in after_comparison),
        )
        ...
        self.assertEqual(2, restarted.preference_version)
