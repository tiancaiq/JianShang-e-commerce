...
Status: implemented as tracked configuration only and reconciled by
`AI-CS-CLEAN-P0-03`. Runtime activation, credentials, deployment, browser
verification, production evidence, cohorts, and rollout remain `BLOCKED`.
After the subsequent bounded session/diagnostic stabilization and mandatory
cleanup, the AI lane is reset to `0/3`.
...
4. Let Flyway apply the Agent-owned forward migrations V1 through V6. Do not
   rewrite or bypass them. The tracked Agent Service dependency waits for the
   migration job to complete successfully and for OpenSearch readiness.
...
Source work is verified locally first and deployed only in explicit batches
owned by a separate deployment task. A source-cleanup checkpoint never edits
deployment environment files, activates flags, or treats an unverified browser
