...
The draft listing form now shows an image selector only after a draft has been
saved. Selecting an allowed image requests a media slot and confirms metadata
through the gateway BFF session. No browser token storage is used.
